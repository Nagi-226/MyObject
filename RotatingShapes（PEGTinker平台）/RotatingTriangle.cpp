...
public:
	...
	// 替换星形生成函数，生成三角形图案
	std::vector<olc::vf2d> MakeTriangleShape(const size_t verts = 3)
	{
		std::vector<olc::vf2d> vOut(verts, { 0,0 });
		float anglestep = 2.0f * 3.14159f / float(verts);
		for (size_t i = 0; i < verts; i++)
			vOut[i] = olc::vf2d(cos(anglestep * float(i)), sin(anglestep * float(i)));
		return vOut;
	}

	std::vector<olc::vf2d> vertsTriangleShape = MakeTriangleShape();

	void DrawTriangleDecal(const olc::vf2d& vPos, const float fRadius, const olc::Pixel colour = olc::WHITE)
	{
		std::vector<olc::vf2d> vDraw(vertsTriangleShape.size(), { 0,0 });
		std::transform(vertsTriangleShape.begin(), vertsTriangleShape.end(), vDraw.begin(),
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		{
			DrawTriangleDecal(vFixedGearPos, fFixedGearRadius, olc::WHITE);
			DrawTriangleDecal(vFixedGearPos + vMovingGearPos, std::abs(fMovingGearRadius), olc::WHITE);
			DrawTriangleDecal(vPenPoint, 4, olc::WHITE);
			DrawLineDecal(
