...
public:
	...
	// 替换图形生成函数，生成正方形图案
	std::vector<olc::vf2d> MakeSquareShape(const size_t verts = 4)
	{
	...
	std::vector<olc::vf2d> vertsSquareShape = MakeSquareShape();

	void DrawSquareDecal(const olc::vf2d& vPos, const float fRadius, const olc::Pixel colour = olc::WHITE)
	{
		std::vector<olc::vf2d> vDraw(vertsSquareShape.size(), { 0,0 });
		std::transform(vertsSquareShape.begin(), vertsSquareShape.end(), vDraw.begin(),
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		{
			DrawSquareDecal(vFixedGearPos, fFixedGearRadius, olc::WHITE);
			DrawSquareDecal(vFixedGearPos + vMovingGearPos, std::abs(fMovingGearRadius), olc::WHITE);
			DrawSquareDecal(vPenPoint, 4, olc::WHITE);
			DrawLineDecal(
