...
void mainWidget::deal_newconnection_slot(qintptr descriptor)
{
    ...
    //子线程发来信号 通知私聊好友
    connect(std,&SoketThread::sigUserSendMsg,this,&mainWidget::on_SigUserSendMsg_slot);
...
void mainWidget::on_SigUserSendMsg_slot(QString jsonStr)
{
    UserData from,to;
    QString msg;
    //解析
    ...
    //找到目的线程(要接收数据的)
     SoketThread *t = users[to];

     QString str =  MsgBuilder::buildUserReceiveMsg(from,to,msg);
     //将消息(数据) 传递给子线程 子线程发送给目的客户端
     t->addUserSendMsg(str);
